...
class ClientArticleLogisticSiteController extends ClientController
{	
	...
	public function article_logistic_site()
	{
		$param = $content = $get = $lang = $article_logistic_site_lang = $current_url = NULL;
		
		if ($_GET) $get = $_GET;
		
		$lang = Lang::get('common');
		$article_logistic_site_lang = Lang::get('client/article_logistic_site');
		$current_url = current_url();
		// debug($article_logistic_site_lang,1);
		
		$param['get'] = $get;
		$param['lang'] = $lang;
		$param['article_logistic_site_lang'] = $article_logistic_site_lang;
		$param['PAGE_TITLE'] = $article_logistic_site_lang['module'];
		$param['MODULE'] = $article_logistic_site_lang['module'];
		
		if (isset($get['do']) && ($get['do'] == 'view' || $get['do'] == 'insert' || $get['do'] == 'edit' && isset($get['article_logistic_site_id']))) {
			if ($get['do'] == 'insert') { 
			...
			$viewtarget = 'client.article_logistic_site_form';
		} else {
			$param['ACTION'] = $lang['list'];
			$viewtarget = 'client.article_logistic_site_list';
		}
		
		$param['PAGE_HEADER'] = $param['ACTION'] . ' ' . $article_logistic_site_lang['module'];
	...
	public function insert()
	{
		...
		if ($_POST)
		{
			...
			$api_url = env('API_URL').'article_logistic_site';
			$api_method = 'post';
	...
	public function update()
	{
		...
		if ($_POST)
		{
            ...
            if (! isset($post['article_logistic_site_id'])) {
                $message = 'article_logistic_site_id not exist';
                return redirect($url_back)->with('message', print_message($message));
            } 
	...
	public function delete()
	{
		...
		if ($_GET)
		{
			...
			// Action start here
			$param = NULL;
			$param['article_logistic_site_id'] = $get['article_logistic_site_id'];
			$param['status'] = -1;
			$param['updated_at'] = get_datetime();
			$param['updated_by'] = 1;
			$param['updated_ip'] = get_ip();
	...
	public function bulk()
	{
		...
		if ($_POST)
		{
			...
			if (isset($save)) {
				...
				// return redirect('article_logistic_site')->with('message', print_message($message));
			}
