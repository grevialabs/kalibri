...
$article_logistic_site_model = new ArticleLogisticSiteModel();
$general_model = new GeneralModel();

$getorder_allowed_list = $article_logistic_site_model->getorder_allowed_list();
...
$api_url = env('API_URL').'article_logistic_site/get_list';
$api_method = 'get';
// $api_header['debug'] = 1;
	...
	<div class="col-sm-12">
		<div class="card">
			<div class="card-body">
				...
				<!-- <a href="<?php //echo $base_url.Request::segment(1).DS.Request::segment(2) . '?do=insert' ?>" class="btn btn-primary btn-sm btninsert"><i class="fa fa-plus" aria-hidden="true"></i> {{ $article_logistic_site_lang['add_new'] }}</a><br/><br/> -->
					...
					<div class="table-responsive">
						<table class="table table-striped table-bordered" id="table_article">
							<tr class="b">
								...
								<td width=1>#</td>
								<td width="150px"><a class="{{ $arrsort['article_logistic_site_id']['class'] }}" title="{{ $arrsort['article_logistic_site_id']['title'] }}" href="{{ $arrsort['article_logistic_site_id']['url'] }}">{{ $article_logistic_site_lang['article_logistic_site_id'] }} {!! $arrsort['article_logistic_site_id']['icon'] !!}</a></td>
								<td width="150px"><a class="{{ $arrsort['site_id']['class'] }}" title="{{ $arrsort['site_id']['title'] }}" href="{{ $arrsort['site_id']['url'] }}">{{ $article_logistic_site_lang['site_id'] }} {!! $arrsort['site_id']['icon'] !!}</a></td>
							...
							if (! empty($listdata)) 
							{
								...
								{
									$i++;
									$id = $rs['article_logistic_site_id'];
									$idcol = 'article_logistic_site_id';
							?>
							
							<tr>
								<td class="parentcheckbox"><input type="checkbox" name="chkbox[]" id="chkbox[]" class="chkbox" value="<?php echo $i?>"/></td>
								<td>{{ $i }}</td>
								<td>{{ $rs['article_logistic_site_id'] or '' }} <br/> <a style="margin-right:6px" href="<?php echo Request::segment(2).'?do=edit&'.$idcol.'='.$id; ?>" title="Edit data" alt="Edit data"><i class="clrBlu fa fa-pencil-square-o fa-lg btnedit"></i></a> </td>
								<td>{{ $rs['site_id'] or '' }}</td>
