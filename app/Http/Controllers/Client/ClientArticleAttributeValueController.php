...
class ClientArticleAttributeValueController extends ClientController
{	
	...
	public function article_attribute_value()
	{
		$param = $content = $get = $lang = $article_attribute_value_lang = $current_url = NULL;
		
		if ($_GET) $get = $_GET;
		
		$lang = Lang::get('common');
		$article_attribute_value_lang = Lang::get('client/article_attribute_value');
		$current_url = current_url();
		// debug($article_attribute_value_lang,1);
		
		$param['get'] = $get;
		$param['lang'] = $lang;
		$param['article_attribute_value_lang'] = $article_attribute_value_lang;
		$param['PAGE_TITLE'] = $article_attribute_value_lang['module'];
		$param['MODULE'] = $article_attribute_value_lang['module'];
		
		if (isset($get['do']) && ($get['do'] == 'view' || $get['do'] == 'insert' || $get['do'] == 'edit' && isset($get['article_attribute_value_id']))) {
			if ($get['do'] == 'insert') { 
			...
			$viewtarget = 'client.article_attribute_value_form';
		} else {
			$param['ACTION'] = $lang['list'];
			$viewtarget = 'client.article_attribute_value_list';
		}
		
		$param['PAGE_HEADER'] = $param['ACTION'] . ' ' . $article_attribute_value_lang['module'];
	...
	public function insert()
	{
		...
		if ($_POST)
		{
			...
			$api_url = env('API_URL').'article_attribute_value';
			$api_method = 'post';
	...
	public function update()
	{
		...
		if ($_POST)
		{
            ...
            if (! isset($post['article_attribute_value_id'])) {
                $message = 'article_attribute_value_id not exist';
                return redirect($url_back)->with('message', print_message($message));
            } 
	...
	public function delete()
	{
		...
		if ($_GET)
		{
			...
			// Action start here
			$param = NULL;
			$param['article_attribute_value_id'] = $get['article_attribute_value_id'];
			$param['status'] = -1;
			$param['updated_at'] = get_datetime();
			$param['updated_by'] = 1;
			$param['updated_ip'] = get_ip();
	...
	public function bulk()
	{
		...
		if ($_POST)
		{
			...
			if (isset($save)) {
				...
				// return redirect('article_attribute_value')->with('message', print_message($message));
			}
