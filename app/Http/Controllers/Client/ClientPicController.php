...
class ClientPicController extends ClientController
{
	...
	public function pic()
	{
		$param = $content = $get = $lang = $piclang = $current_url = NULL;
		
		if ($_GET) $get = $_GET;
		
		$lang = Lang::get('common');
		$piclang = Lang::get('client/pic');
		$current_url = current_url();
		// debug($piclang,1);
		
		$param['get'] = $get;
		$param['lang'] = $lang;
		$param['piclang'] = $piclang;
		$param['PAGE_TITLE'] = $piclang['module'];
		$param['MODULE'] = $piclang['module'];
		
		if (isset($get['do']) && ($get['do'] == 'insert' || $get['do'] == 'edit' && isset($get['pic_id']))) {
			if ($get['do'] == 'insert') { 
			...
			$viewtarget = 'client.pic_form';
		} else {
			$param['ACTION'] = $lang['list'];
			$viewtarget = 'client.pic_list';
		}
		
		$param['PAGE_HEADER'] = $param['ACTION'] . ' ' . $piclang['module'];
	...
	public function update()
	{
		...
		if ($_POST)
		{
            ...
            if (! isset($post['pic_id'])) {
                $message = 'pic_id not exist';
                return redirect($url_back)->with('message', print_message($message));
            } 
	...
	public function bulk()
	{
		...
		if ($_POST)
		{
			...
			if (isset($save)) {
				...
				// return redirect('pic')->with('message', print_message($message));
			}
