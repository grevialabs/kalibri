...
class ClientPrepackBundlingDetailController extends ClientController
{	
	...
	public function prepack_bundling_detail()
	{
		$param = $content = $get = $lang = $prepack_bundling_detail_lang = $current_url = NULL;
		
		if ($_GET) $get = $_GET;
		
		$lang = Lang::get('common');
		$prepack_bundling_detail_lang = Lang::get('client/prepack_bundling_detail');
		$current_url = current_url();
		// debug($prepack_bundling_detail_lang,1);
		
		$param['get'] = $get;
		$param['lang'] = $lang;
		$param['prepack_bundling_detail_lang'] = $prepack_bundling_detail_lang;
		$param['PAGE_TITLE'] = $prepack_bundling_detail_lang['module'];
		$param['MODULE'] = $prepack_bundling_detail_lang['module'];
		
		if (isset($get['do']) && ($get['do'] == 'view' || $get['do'] == 'insert' || $get['do'] == 'edit' && isset($get['prepack_bundling_detail_id']))) {
			if ($get['do'] == 'insert') { 
			...
			$viewtarget = 'client.prepack_bundling_detail_form';
		} else {
			$param['ACTION'] = $lang['list'];
			$viewtarget = 'client.prepack_bundling_detail_list';
		}
		
		$param['PAGE_HEADER'] = $param['ACTION'] . ' ' . $prepack_bundling_detail_lang['module'];
	...
	public function insert()
	{
		...
		if ($_POST)
		{
			...
			$api_url = env('API_URL').'prepack_bundling_detail';
			$api_method = 'post';
	...
	public function update()
	{
		...
		if ($_POST)
		{
            ...
            if (! isset($post['prepack_bundling_detail_id'])) {
                $message = 'prepack_bundling_detail_id not exist';
                return redirect($url_back)->with('message', print_message($message));
            } 
	...
	public function delete()
	{
		...
		if ($_GET)
		{
			...
			// Action start here
			$param = NULL;
			$param['prepack_bundling_detail_id'] = $get['prepack_bundling_detail_id'];
			$param['status'] = -1;
			$param['updated_at'] = get_datetime();
			$param['updated_by'] = 1;
			$param['updated_ip'] = get_ip();
	...
	public function bulk()
	{
		...
		if ($_POST)
		{
			...
			if (isset($save)) {
				...
				// return redirect('prepack_bundling_detail')->with('message', print_message($message));
			}
