...
$article_po_history_model = new ArticlePoHistoryModel();
$general_model = new GeneralModel();

$getorder_allowed_list = $article_po_history_model->getorder_allowed_list();
...
$api_url = env('API_URL').'article_po_history/get_list';
$api_method = 'get';
// $api_header['debug'] = 1;
	...
	<div class="col-sm-12">
		<div class="card">
			<div class="card-body">
				...
				<!-- <a href="<?php //echo $base_url.Request::segment(1).DS.Request::segment(2) . '?do=insert' ?>" class="btn btn-primary btn-sm insert"><i class="fa fa-plus" aria-hidden="true"></i> {{ $article_po_history_lang['add_new'] }}</a><br/><br/> -->
					...
					<div class="table-responsive">
						<table class="table table-striped table-bordered" id="table_pic">
							<tr class="b">
								...
								<td width=1>#</td>
								<td width="150px"><a class="{{ $arrsort['article_po_history_id']['class'] }}" title="{{ $arrsort['article_po_history_id']['title'] }}" href="{{ $arrsort['article_po_history_id']['url'] }}">{{ $article_po_history_lang['article_po_history_id'] }} {!! $arrsort['article_po_history_id']['icon'] !!}</a></td>
								<td width="150px"><a class="{{ $arrsort['article_po_id']['class'] }}" title="{{ $arrsort['article_po_id']['title'] }}" href="{{ $arrsort['article_po_id']['url'] }}">{{ $article_po_history_lang['article_po_id'] }} {!! $arrsort['article_po_id']['icon'] !!}</a></td>
								<td width="180px"><a class="{{ $arrsort['po_usage_qty']['class'] }}" title="{{ $arrsort['po_usage_qty']['title'] }}" href="{{ $arrsort['po_usage_qty']['url'] }}">{{ $article_po_history_lang['po_usage_qty'] }} {!! $arrsort['po_usage_qty']['icon'] !!}</a></td>
								<td width="180px"><a class="{{ $arrsort['po_remaining_qty']['class'] }}" title="{{ $arrsort['po_remaining_qty']['title'] }}" href="{{ $arrsort['po_remaining_qty']['url'] }}">{{ $article_po_history_lang['po_remaining_qty'] }} {!! $arrsort['po_remaining_qty']['icon'] !!}</a></td>
                                <td width="180px"><a class="{{ $arrsort['po_created_date']['class'] }}" title="{{ $arrsort['po_created_date']['title'] }}" href="{{ $arrsort['po_created_date']['url'] }}">{{ $article_po_history_lang['po_created_date'] }} {!! $arrsort['po_created_date']['icon'] !!}</a></td>
                                <td width="180px"><a class="{{ $arrsort['status_in_out']['class'] }}" title="{{ $arrsort['status_in_out']['title'] }}" href="{{ $arrsort['status_in_out']['url'] }}">{{ $article_po_history_lang['status_in_out'] }} {!! $arrsort['status_in_out']['icon'] !!}</a></td>
							...
							if (! empty($listdata)) 
							{
								...
								{
									$i++;
									$id = $rs['article_po_history_id'];
									$idcol = 'article_po_history_id';
							?>
							
							<tr>
								<td class="parentcheckbox"><input type="checkbox" name="chkbox[]" id="chkbox[]" class="chkbox" value="<?php echo $i?>"/></td>
								<td>{{ $i }}</td>
								<td>{{ $rs['article_po_history_id'] }}  <br/> <a style="margin-right:6px" href="<?php echo Request::segment(2).'?do=edit&'.$idcol.'='.$id; ?>" title="Edit data" alt="Edit data"><i class="clrBlu fa fa-pencil-square-o fa-lg btnedit"></i></a> </td>
								<td>{{ $rs['article_po_id'] }}</td>
								<td>{{ $rs['po_usage_qty'] }}</td>
								<td>{{ $rs['po_remaining_qty'] }}</td>							
								<td>{{ $rs['po_created_date'] }}</td>							
								<td>{{ $rs['status_in_out'] }}</td>							
