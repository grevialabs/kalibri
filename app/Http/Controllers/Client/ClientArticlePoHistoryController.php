...
class ClientArticlePoHistoryController extends ClientController
{	
	...
	public function article_po_history()
	{
		$param = $content = $get = $lang = $article_po_history_lang = $current_url = NULL;
		
		if ($_GET) $get = $_GET;
		
		$lang = Lang::get('common');
		$article_po_history_lang = Lang::get('client/article_po_history');
		$current_url = current_url();
		// debug($article_po_history_lang,1);
		
		$param['get'] = $get;
		$param['lang'] = $lang;
		$param['article_po_history_lang'] = $article_po_history_lang;
		$param['PAGE_TITLE'] = $article_po_history_lang['module'];
		$param['MODULE'] = $article_po_history_lang['module'];
		
		if (isset($get['do']) && ($get['do'] == 'view' || $get['do'] == 'insert' || $get['do'] == 'edit' && isset($get['article_po_history_id']))) {
			if ($get['do'] == 'insert') { 
			...
			$viewtarget = 'client.article_po_history_form';
		} else {
			$param['ACTION'] = $lang['list'];
			$viewtarget = 'client.article_po_history_list';
		}
		
		$param['PAGE_HEADER'] = $param['ACTION'] . ' ' . $article_po_history_lang['module'];
	...
	public function insert()
	{
		...
		if ($_POST)
		{
			...
			$api_url = env('API_URL').'article_po_history';
			$api_method = 'post';
	...
	public function update()
	{
		...
		if ($_POST)
		{
            ...
            if (! isset($post['article_po_history_id'])) {
                $message = 'article_po_history_id not exist';
                return redirect($url_back)->with('message', print_message($message));
            } 
	...
	public function delete()
	{
		...
		if ($_GET)
		{
			...
			// Action start here
			$param = NULL;
			$param['article_po_history_id'] = $get['article_po_history_id'];
			$param['status'] = -1;
			$param['updated_at'] = get_datetime();
			$param['updated_by'] = 1;
			$param['updated_ip'] = get_ip();
	...
	public function bulk()
	{
		...
		if ($_POST)
		{
			...
			if (isset($save)) {
				...
				// return redirect('article_po_history')->with('message', print_message($message));
			}
