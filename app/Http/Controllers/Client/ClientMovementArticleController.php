...
class ClientMovementArticleController extends ClientController
{
	...
	public function movement_article()
	{
		$param = $content = $get = $lang = $movement_article_lang = $current_url = NULL;
		
		if ($_GET) $get = $_GET;
		
		$lang = Lang::get('common');
		$movement_article_lang = Lang::get('client/movement_article');
		$current_url = current_url();
		//  debug($movement_article_lang,1);
		
		$param['get'] = $get;
		$param['lang'] = $lang;
		$param['movement_article_lang'] = $movement_article_lang;
		$param['PAGE_TITLE'] = $movement_article_lang['module'];
		$param['MODULE'] = $movement_article_lang['module'];
		
		if (isset($get['do']) && ($get['do'] == 'insert' || $get['do'] == 'edit' && isset($get['movement_article_id']))) {
			if ($get['do'] == 'insert') { 
		...
		$param['PAGE_HEADER'] = $param['ACTION'] . ' ' . $movement_article_lang['module'];
	...
	public function insert()
	{
		...
		if ($_POST)
		{
			...
			$api_url = env('API_URL').'movement_article';
			$api_method = 'post';
	...
	public function update()
	{
		...
		if ($_POST)
		{
            ...
            if (! isset($post['movement_article_id'])) {
                $message = 'movement_article_id not exist';
                return redirect($url_back)->with('message', print_message($message));
            } 
	...
	public function delete()
	{
		...
		if ($_GET)
		{
			...
			// Action start here
			$param = NULL;
			$param['movement_article_id'] = $get['movement_article_id'];
			$param['status'] = -1;
			$param['updated_at'] = get_datetime();
			$param['updated_by'] = 1;
			$param['updated_ip'] = get_ip();
	...
	public function bulk()
	{
		...
		if ($_POST)
		{
			...
			if (isset($save)) {
				...
				// return redirect('movement_article')->with('message', print_message($message));
			}
