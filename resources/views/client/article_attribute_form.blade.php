...
if (isset($get['article_attribute_id'])) {
	...
	$api_param['article_attribute_id'] = $get['article_attribute_id'];

	$api_url = env('API_URL').'article_attribute/get';
	$api_method = 'get';
	// $api_header['debug'] = 1;
				...
				<form method="post" action="{{ $form_url }}" class="form-horizontal">
					...
					<div class="form-group row">
						...
						<div class="col-lg-7 col-lg-offset-3 col-md-9 col-sm-12">
							<input type="text" data-toggle="" title="" class="form-control" id="" placeholder="{{ $article_attribute_lang['article_attribute_id'] }}" required="" data-original-title="" value="{{ $data['article_attribute_id'] }}" disabled />
							<input type="hidden" name="article_attribute_id" value="{{ $data['article_attribute_id'] }}" />
						</div>
					</div>
					<?php } ?>
					...
					<div class="form-group row">
						<div class="col-lg-2 col-md-3 col-sm-12">
							<label for="attribute_name" class="control-label col-form-label">{!! $article_attribute_lang['attribute_name'] !!}</label>
						</div>
						<div class="col-lg-7 col-lg-offset-3 col-md-9 col-sm-12">
							<input type="text" data-toggle="{{ $article_attribute_lang['attribute_name'] }}" title="{{ $article_attribute_lang['attribute_name'] }}" class="form-control" id="attribute_name" name="attribute_name" placeholder="{{ $article_attribute_lang['attribute_name'] }}" required="" data-original-title="" />
						</div>
					</div>
