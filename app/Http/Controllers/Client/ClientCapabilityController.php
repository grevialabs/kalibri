...
class ClientCapabilityController extends ClientController
{	
	...
	{
		$param = $content = $get = $lang = $capabilitylang = $current_url = NULL;
		
		if ($_GET) $get = $_GET;
		
		$lang = Lang::get('common');
		$capabilitylang = Lang::get('client/capability');
		$current_url = current_url();
		// debug($capabilitylang,1);
		
		$param['get'] = $get;
		$param['lang'] = $lang;
		$param['capabilitylang'] = $capabilitylang;
		$param['PAGE_TITLE'] = $capabilitylang['module'];
		$param['MODULE'] = $capabilitylang['module'];
		
		if (isset($get['do']) && ($get['do'] == 'view' || $get['do'] == 'insert' || $get['do'] == 'edit' && isset($get['capability_id']))) {
			if ($get['do'] == 'insert') { 
			...
			$viewtarget = 'client.capability_form';
		} else {
			$param['ACTION'] = $lang['list'];
			$viewtarget = 'client.capability_list';
		}
		
		$param['PAGE_HEADER'] = $param['ACTION'] . ' ' . $capabilitylang['module'];
	...
	public function insert()
	{
		...
		if ($_POST)
		{
			...
			$api_url = env('API_URL').'capability';
			$api_method = 'post';
	...
	public function update()
	{
		...
		if ($_POST)
		{
            ...
            if (! isset($post['capability_id'])) {
                $message = 'capability_id not exist';
                return redirect($url_back)->with('message', print_message($message));
            } 
	...
	public function delete()
	{
		...
		if ($_GET)
		{
			...
			// Action start here
			$param = NULL;
			$param['capability_id'] = $get['capability_id'];
			$param['status'] = -1;
			$param['updated_at'] = get_datetime();
			$param['updated_by'] = 1;
			$param['updated_ip'] = get_ip();
	...
	public function bulk()
	{
		...
		if ($_POST)
		{
			...
			if (isset($save)) {
				...
				// return redirect('capability')->with('message', print_message($message));
			}
