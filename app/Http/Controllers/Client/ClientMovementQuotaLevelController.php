...
class ClientMovementQuotaLevelController extends ClientController
{
	...
	public function movement_quota_level()
	{
		$param = $content = $get = $lang = $movement_quota_level_lang = $current_url = NULL;
		
		if ($_GET) $get = $_GET;
		
		$lang = Lang::get('common');
		$movement_quota_level_lang = Lang::get('client/movement_quota_level');
		$current_url = current_url();
		//  debug($movement_quota_level_lang,1);
		
		$param['get'] = $get;
		$param['lang'] = $lang;
		$param['movement_quota_level_lang'] = $movement_quota_level_lang;
		$param['PAGE_TITLE'] = $movement_quota_level_lang['module'];
		$param['MODULE'] = $movement_quota_level_lang['module'];
		
		if (isset($get['do']) && ($get['do'] == 'insert' || $get['do'] == 'edit' && isset($get['movement_quota_level_id']))) {
			if ($get['do'] == 'insert') { 
		...
		$param['PAGE_HEADER'] = $param['ACTION'] . ' ' . $movement_quota_level_lang['module'];
	...
	public function insert()
	{
		...
		if ($_POST)
		{
			...
			// $param['pic_token'] = env('API_KEY');
			
			$api_url = env('API_URL').'movement_quota_level';
			$api_method = 'post';
	...
	public function update()
	{
		...
		if ($_POST)
		{
            ...
            if (! isset($post['movement_quota_level_id'])) {
                $message = 'movement_quota_level_id not exist';
                return redirect($url_back)->with('message', print_message($message));
            } 
	...
	public function delete()
	{
		...
		if ($_GET)
		{
			...
			// Action start here
			$param = NULL;
			$param['movement_quota_level_id'] = $get['movement_quota_level_id'];
			$param['status'] = -1;
			$param['updated_at'] = get_datetime();
			$param['updated_by'] = 1;
			$param['updated_ip'] = get_ip();
	...
	public function bulk()
	{
		...
		if ($_POST)
		{
			...
			if (isset($save)) {
				...
				// return redirect('movement_quota_level')->with('message', print_message($message));
			}
