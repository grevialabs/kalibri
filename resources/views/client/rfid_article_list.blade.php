...
$rfid_article_model = new RfidArticleModel();
$general_model = new GeneralModel();

$getorder_allowed_list = $rfid_article_model->getorder_allowed_list();
...
$api_url = env('API_URL').'rfid_article/get_list';
$api_method = 'get';
// $api_header['debug'] = 1;
	...
	<div class="col-sm-12">
		<div class="card">
			<div class="card-body">
				...
				<a href="<?php echo $base_url.Request::segment(1).DS.Request::segment(2) . '?do=insert' ?>" class="btn btn-primary btn-sm insert"><i class="fa fa-plus" aria-hidden="true"></i> {{ $rfid_article_lang['add_new'] }}</a><br/><br/>
					...
					<div class="table-responsive">
						<table class="table table-striped table-bordered" id="table_reason">
							<tr class="b">
								...
								<td width=1>#</td>
								<td width="150px"><a class="{{ $arrsort['rfid_article_id']['class'] }}" title="{{ $arrsort['rfid_article_id']['title'] }}" href="{{ $arrsort['rfid_article_id']['url'] }}">{{ $rfid_article_lang['rfid_article_id'] }} {!! $arrsort['rfid_article_id']['icon'] !!}</a></td>
								<td width="150px"><a class="{{ $arrsort['site_id']['class'] }}" title="{{ $arrsort['site_id']['title'] }}" href="{{ $arrsort['site_id']['url'] }}">{{ $rfid_article_lang['site_id'] }} {!! $arrsort['site_id']['icon'] !!}</a></td>
								<td width="150px"><a class="{{ $arrsort['outbound_delivery']['class'] }}" title="{{ $arrsort['outbound_delivery']['title'] }}" href="{{ $arrsort['outbound_delivery']['url'] }}">{{ $rfid_article_lang['outbound_delivery'] }} {!! $arrsort['outbound_delivery']['icon'] !!}</a></td>
								<td width="150px"><a class="{{ $arrsort['article']['class'] }}" title="{{ $arrsort['article']['title'] }}" href="{{ $arrsort['article']['url'] }}">{{ $rfid_article_lang['article'] }} {!! $arrsort['article']['icon'] !!}</a></td>
								<td width="150px"><a class="{{ $arrsort['description']['class'] }}" title="{{ $arrsort['description']['title'] }}" href="{{ $arrsort['description']['url'] }}">{{ $rfid_article_lang['description'] }} {!! $arrsort['description']['icon'] !!}</a></td>
								<td width="150px"><a class="{{ $arrsort['rfid']['class'] }}" title="{{ $arrsort['rfid']['title'] }}" href="{{ $arrsort['rfid']['url'] }}">{{ $rfid_article_lang['rfid'] }} {!! $arrsort['rfid']['icon'] !!}</a></td>
								<td width="150px"><a class="{{ $arrsort['picktime']['class'] }}" title="{{ $arrsort['picktime']['title'] }}" href="{{ $arrsort['picktime']['url'] }}">{{ $rfid_article_lang['picktime'] }} {!! $arrsort['picktime']['icon'] !!}</a></td>
								<td width="150px"><a class="{{ $arrsort['user_id']['class'] }}" title="{{ $arrsort['user_id']['title'] }}" href="{{ $arrsort['user_id']['url'] }}">{{ $rfid_article_lang['user_id'] }} {!! $arrsort['user_id']['icon'] !!}</a></td>
								<!-- <td width="150px"><a class="{{ $arrsort['sync_date']['class'] }}" title="{{ $arrsort['sync_date']['title'] }}" href="{{ $arrsort['sync_date']['url'] }}">{{ $rfid_article_lang['sync_date'] }} {!! $arrsort['sync_date']['icon'] !!}</a></td> -->
							...
							if (! empty($listdata)) 
							{
								...
								{
									$i++;
									$id = $rs['rfid_article_id'];
									$idcol = 'rfid_article_id';
							?>
							
							<tr>
								<td class="parentcheckbox"><input type="checkbox" name="chkbox[]" id="chkbox[]" class="chkbox" value="<?php echo $i?>"/></td>
								<td>{{ $i }}</td>
								<td>{{ $rs['rfid_article_id'] }}  <br/> <a style="margin-right:6px" href="<?php echo Request::segment(2).'?do=edit&'.$idcol.'='.$id; ?>" title="Edit data" alt="Edit data"><i class="clrBlu fa fa-pencil-square-o fa-lg btnedit"></i></a> </td>
								<td>{{ $rs['site_id'] }}</td>
								<td>{{ $rs['outbound_delivery'] }}</td>
								<td>{{ $rs['article'] }}</td>
