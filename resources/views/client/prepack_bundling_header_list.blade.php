...
$prepack_bundling_header_model = new PrepackBundlingHeaderModel();
$general_model = new GeneralModel();

$getorder_allowed_list = $prepack_bundling_header_model->getorder_allowed_list();
...
$api_url = env('API_URL').'prepack_bundling_header/get_list';
$api_method = 'get';
 //$api_header['debug'] = 1;
	...
	<div class="col-sm-12">
		<div class="card">
			<div class="card-body">
				...
				<a href="<?php echo $base_url.Request::segment(1).DS.Request::segment(2) . '?do=insert' ?>" class="btn btn-primary btn-sm insert"><i class="fa fa-plus" aria-hidden="true"></i> {{ $prepack_bundling_header_lang['add_new'] }}</a><br/><br/>
					...
					<div class="table-responsive">
						<table class="table table-striped table-bordered" id="table_site">
							<tr class="b">
								...
								<td width=1>#</td>
								<td width="150px"><a class="{{ $arrsort['prepack_id']['class'] or '' }}" title="{{ $arrsort['prepack_id']['title'] or '' }}" href="{{ $arrsort['prepack_id']['url'] or '' }}">{{ $prepack_bundling_header_lang['prepack_id'] or '' }} {!! $arrsort['prepack_id']['icon'] !!}</a></td>
								<td width="180px"><a class="{{ $arrsort['outbound_delivery']['class'] or '' }}" title="{{ $arrsort['outbound_delivery']['title'] or '' }}" href="{{ $arrsort['outbound_delivery']['url'] or '' }}">{{ $prepack_bundling_header_lang['outbound_delivery'] or '' }} {!! $arrsort['outbound_delivery']['icon'] !!}</a></td>
								<td width="180px"><a class="{{ $arrsort['site_created_on']['class'] or '' }}" title="{{ $arrsort['site_created_on']['title'] or '' }}" href="{{ $arrsort['site_created_on']['url'] or '' }}">{{ $prepack_bundling_header_lang['site_created_on'] or '' }} {!! $arrsort['site_created_on']['icon'] or '' !!}</a></td>
								<td width="180px"><a class="{{ $arrsort['status_prepack']['class'] or '' }}" title="{{ $arrsort['status_prepack']['title'] or '' }}" href="{{ $arrsort['status_prepack']['url'] or '' }}">{{ $prepack_bundling_header_lang['status_prepack'] or '' }}  {!! $arrsort['status_prepack']['icon'] or '' !!}</a></td>
								<td width="180px"><a class="{{ $arrsort['conv_uom']['class'] or '' }}" title="{{ $arrsort['conv_uom']['title'] or '' }}" href="{{ $arrsort['conv_uom']['url'] or '' }}">{{ $prepack_bundling_header_lang['conv_uom'] or '' }} {!! $arrsort['conv_uom']['icon'] or '' !!}</a></td>
								<td width="180px"><a class="{{ $arrsort['combine_qty']['class'] or '' }}" title="{{ $arrsort['combine_qty']['title'] or '' }}" href="{{ $arrsort['combine_qty']['url'] or '' }}">{{ $prepack_bundling_header_lang['combine_qty'] or '' }} {!! $arrsort['combine_qty']['icon'] or '' !!}</a></td>
								<td width="180px"><a class="{{ $arrsort['combine_qty']['class'] or '' }}" title="{{ $arrsort['combine_qty']['title'] or '' }}" href="{{ $arrsort['combine_qty']['url'] or '' }}">{{ $prepack_bundling_header_lang['combine_qty'] or '' }} {!! $arrsort['combine_qty']['icon'] or '' !!}</a></td>
							...
							<tr>
								...
								<td>{{ $i }}</td>
								<td>{{ $rs['prepack_id'] or '' }} <br/> <a style="margin-right:6px" href="<?php echo Request::segment(2).'?do=edit&'.$idcol.'='.$id; ?>" title="Edit data" alt="Edit data"><i class="clrBlu fa fa-pencil-square-o fa-lg btnedit"></i></a> </td>
								<td>{{ $rs['outbound_delivery'] or '' }}</td>
								<td>{{ $rs['site_created_on'] or '' }}</td>
								<td>{{ $rs['status_prepack'] or '' }}</td>
								<td>{{ $rs['conv_uom'] or '' }}</td>
								<td>{{ $rs['combine_qty'] or '' }}</td>
