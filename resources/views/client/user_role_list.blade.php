...
$getorder_allowed_list = $user_role_model->getorder_allowed_list();
...
$api_url = env('API_URL').'user_role/get_list';
$api_method = 'get';
// $api_header['debug'] = 1;
	...
	<div class="col-sm-12">
		<div class="card">
			<div class="card-body">
				...
				<a href="<?php echo $base_url.Request::segment(1).DS.Request::segment(2) . '?do=insert' ?>" class="btn btn-primary btn-sm insert"><i class="fa fa-plus" aria-hidden="true"></i> {{ $user_role_lang['add_new'] }}</a><br/><br/>
					...
					<div class="table-responsive">
						<table class="table table-striped table-bordered" id="table_user_attribute">
							<tr class="b">
								...
								<td width=1>#</td>
								<td width="150px"><a class="{{ $arrsort['user_role_id']['class'] }}" title="{{ $arrsort['user_role_id']['title'] }}" href="{{ $arrsort['user_role_id']['url'] }}">{{ $user_role_lang['user_role_id'] }} {!! $arrsort['user_role_id']['icon'] !!}</a></td>
								<td width="180px"><a class="{{ $arrsort['role_id']['class'] }}" title="{{ $arrsort['role_id']['title'] }}" href="{{ $arrsort['role_id']['url'] }}">{{ $user_role_lang['role_id'] }} {!! $arrsort['role_id']['icon'] !!}</a></td>
								<td width="150px"><a class="{{ $arrsort['user_id']['class'] }}" title="{{ $arrsort['user_id']['title'] }}" href="{{ $arrsort['user_id']['url'] }}">{{ $user_role_lang['user_id'] }} {!! $arrsort['user_id']['icon'] !!}</a></td>
							...
							<tr>
								...
								<td>{{ $i }}</td>
								<td>{{ $rs['user_role_id'] }} <br/> <a style="margin-right:6px" href="<?php echo Request::segment(2).'?do=edit&'.$idcol.'='.$id; ?>" title="Edit data" alt="Edit data"><i class="clrBlu fa fa-pencil-square-o fa-lg btnedit"></i></a> </td>
								<td>{{ $rs['role_id'] or ''}}</td>
								<td>{{ $rs['user_id'] or ''}}</td>
