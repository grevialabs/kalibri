...
$article_po_model = new ArticlePoModel();
$general_model = new GeneralModel();

$getorder_allowed_list = $article_po_model->getorder_allowed_list();
...
$api_url = env('API_URL').'article_po/get_list';
$api_method = 'get';
// $api_header['debug'] = 1;
	...
	<div class="col-sm-12">
		<div class="card">
			<div class="card-body">
				...
				<a href="<?php echo $base_url.Request::segment(1).DS.Request::segment(2) . '?do=insert' ?>" class="btn btn-primary btn-sm insert"><i class="fa fa-plus" aria-hidden="true"></i> {{ $article_po_lang['add_new'] }}</a><br/><br/>
					...
					<div class="table-responsive">
						<table class="table table-striped table-bordered" id="table_pic">
							<tr class="b">
								...
								<td width=1>#</td>
								<td width="150px"><a class="{{ $arrsort['article_po_id']['class'] }}" title="{{ $arrsort['article_po_id']['title'] }}" href="{{ $arrsort['article_po_id']['url'] }}">{{ $article_po_lang['article_po_id'] }} {!! $arrsort['article_po_id']['icon'] !!}</a></td>
								<td width="150px"><a class="{{ $arrsort['site_id']['class'] }}" title="{{ $arrsort['site_id']['title'] }}" href="{{ $arrsort['site_id']['url'] }}">{{ $article_po_lang['site_id'] }} {!! $arrsort['site_id']['icon'] !!}</a></td>
								<td width="180px"><a class="{{ $arrsort['article']['class'] }}" title="{{ $arrsort['article']['title'] }}" href="{{ $arrsort['article']['url'] }}">{{ $article_po_lang['article'] }} {!! $arrsort['article']['icon'] !!}</a></td>
								<td width="180px"><a class="{{ $arrsort['customer_article']['class'] }}" title="{{ $arrsort['customer_article']['title'] }}" href="{{ $arrsort['customer_article']['url'] }}">{{ $article_po_lang['customer_article'] }} {!! $arrsort['customer_article']['icon'] !!}</a></td>
                                <td width="180px"><a class="{{ $arrsort['description']['class'] }}" title="{{ $arrsort['description']['title'] }}" href="{{ $arrsort['description']['url'] }}">{{ $article_po_lang['description'] }} {!! $arrsort['description']['icon'] !!}</a></td>
							...
							<tr>
								...
								<td>{{ $i }}</td>
								<td>{{ $rs['article_po_id'] }}  <br/> <a style="margin-right:6px" href="<?php echo Request::segment(2).'?do=edit&'.$idcol.'='.$id; ?>" title="Edit data" alt="Edit data"><i class="clrBlu fa fa-pencil-square-o fa-lg btnedit"></i></a> </td>
								<td>{{ $rs['site_id'] }}</td>
								<td>{{ $rs['article'] }}</td>
								<td>{{ $rs['customer_article'] }}</td>							
								<td>{{ $rs['description'] }}</td>							
								<td>{{ $rs['po_blanket_number'] }}</td>							
								<td>{{ $rs['po_created_date'] }}</td>							
								<td>{{ $rs['po_blanket_qty'] }}</td>							
